...
namespace MvcPaging
{
    ...
    public class PagedString : IPagedString
    {
        

        public PagedString(string source, int index, int pageSize)
        {
            if (index < 0)
                throw new ArgumentOutOfRangeException("index", "Value can not be below 0.");
            if (pageSize < 1)
            ...
            if (TotalItemCount <= 0)
                return;
            CurrentPageContent = source.Substring(ItemStart, ItemEnd-ItemStart);
        }
      

        public string CurrentPageContent { get; private set; }
